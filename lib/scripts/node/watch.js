...
function enableWatch() {
  var defaults = loadH2Defaults('prod');
  var config = loadH2Config('prod');
  watch(['./hydrogen.config.json'], series(buildSeries));
  var codeArray = [];
  function getWatchUserMarkup() {
    if (Array.isArray(config.folders.markup) == true) {
      config.folders.markup.forEach(function(path, index, array) {
        codeArray = codeArray.concat('./' + path + '/**/*');
      });
    } else {
      codeArray = codeArray.concat('./' + config.folders.markup + '/**/*');
    }
    if (Array.isArray(config.folders.scripts) == true) {
      ...
      codeArray = codeArray.concat('./' + config.folders.scripts + '/**/*');
    }
  }
  getWatchUserMarkup();
