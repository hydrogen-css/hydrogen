// Hydrogen.css / Production Watch Script
...
function successMessage(done) {
  ...
  console.log(
    'H2',
    '[WATCHING]'.rainbow,
    'Hydrogen is now watching for changes to your code...'
  );
  done();
}
  ...
  function getWatchUserMarkup() {
    if (Array.isArray(config.folders.markup) == true) {
      ...
      codeArray = codeArray.concat(
        './lib/stage/' + config.folders.markup + '/**/*'
      );
    }
    if (Array.isArray(config.folders.scripts) == true) {
      ...
      codeArray = codeArray.concat(
        './lib/stage/' + config.folders.scripts + '/**/*'
      );
    }
  }
  getWatchUserMarkup();
