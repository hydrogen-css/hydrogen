...
function createHydrogenInit(done) {
  if (fs.existsSync('./lib/stage/' + markup) == true) {
    if (fs.existsSync('./lib/stage/' + scripts) == true ) {
      if (fs.existsSync('./lib/stage/' + styles) == true) {
        ...
        // hydrogenInit = '{\n  "folders": {\n    "markup": "' + markup + '",\n    "scripts": "' + scripts + '",\n    "styles": "' + styles + '"\n  }\n}';
        // Write the file.
        fs.writeFile('./lib/stage/hydrogen.config.json', JSON.stringify(defaults, null, 2), function(err) {
          if (err) {
