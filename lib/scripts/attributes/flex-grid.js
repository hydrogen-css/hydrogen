 ...
 */
function parseFlexGrid(property, selector, values) {
  try {
    ...
    var wrapperPadding = parseWhitespace(property, values[1]);
    var wrapperPaddingErrorMessage =
      '"'.red +
      wrapperPadding.red +
      '"'.red +
      ' is not a valid space option for ' +
      property[0].underline +
      '.';
    var itemGutter = parseWhitespace(property, values[2]);
    var itemGutterErrorMessage =
      '"'.red +
      itemGutter.red +
      '"'.red +
      ' is not a valid space option for ' +
    ...
    if (values.length == 3) {
      ...
      if (wrapperPadding == null) {
        h2_error(wrapperPaddingErrorMessage);
        throw 'Error';
      } else if (itemGutter == null) {
        h2_error(itemGutterErrorMessage);
        throw 'Error';
      } else {
      ...
      var rowGutter = parseWhitespace(property, values[3]);
      var rowGutterErrorMessage =
        '"'.red +
        rowGutter.red +
        '"'.red +
        ' is not a valid space option for ' +
      ...
      if (wrapperPadding == null) {
        ...
        h2_error(rowGutterErrorMessage);
        throw 'Error';
      } else {
        // Set up the CSS string
        var cssString =
      ...
      var errorMessage =
        ...
        property[0].red +
        '"'.red +
        " accepts 3 or 4 values, and you've specified " +
        values.length +
        '.';
