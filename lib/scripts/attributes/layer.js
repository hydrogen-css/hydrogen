 ...
 */
function parseLayer(argv, property, selector, values) {
  try {
    ...
    if (values.length >= 1) {
      layer = values[0];
      layerErr = '"'.red + layer.red + '"'.red + ' is an invalid option for ' + property[0].underline + '.';
    }
    ...
    if (values.length == 2) {
      position = values[1];
      positionErr = '"'.red + position.toString().red + '"'.red + ' is an invalid option for ' + property[0].underline + '.';
    }
