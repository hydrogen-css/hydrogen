 ...
 */
function parseOffset(argv, property, selector, values) {
  try {
    ...
    var value1Err;
    if (values.length >= 1) {
      value1 = parseWhitespace(argv, property, values[0]);
    ...
    var value2Err;
    if (values.length >= 2) {
      value2 = parseWhitespace(argv, property, values[1]);
    ...
    var value3Err;
    if (values.length >= 3) {
      value3 = parseWhitespace(argv, property, values[2]);
    ...
    var value4Err;
    if (values.length >= 4) {
      value4 = parseWhitespace(argv, property, values[3]);
    ...
    if (values.length == 1) {
      if (value1 == null) {
        ...
        cssString = '{top: ' + value1 + ';right: ' + value1 + ';bottom: ' + value1 + ';left: ' + value1 + ';transition: all 0.2s ease;}';
      }
      ...
      if (value1 == null) {
        ...
        cssString = '{top: ' + value1 + ';right: ' + value2 + ';bottom: ' + value1 + ';left: ' + value2 + ';transition: all 0.2s ease;}';
      }
