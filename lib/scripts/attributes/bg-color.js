  ...
  try {
    ...
    if (values.length == 1) {
      ...
      if (bgColor == null) {
        ...
        var errorMessage = '"'.red + values[0].red + '"'.red + ' is an invalid color for ' + property[0].underline + '.';
        ...
        if (bgColor.type == 'solid') {
          cssString = '{background-color: ' + bgColor.color + ';transition: 0.2s ease all;}';
        } else if (bgColor.type == 'gradient') {
          cssString = '{background-color: ' + bgColor.fallback + ';background-image: ' + bgColor.color + ';transition: 0.2s ease all;}';
        }
