 ...
 */
function parseOverlay(property, selector, values) {
  try {
    ...
    var bgColorErr;
    if (values.length >= 1) {
      bgColor = parseColor(property, values[0]);
    ...
    var attributeCSS = [selector + ':before' + cssString, selector + '{position: relative;}', selector + ' > * {position: relative;}'];
    // Return the array
    return attributeCSS;
  } catch (err) {
    return null;
