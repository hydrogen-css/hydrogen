...
function parseShadow(property, selector, values) {
  try {
    ...
    if (values.length >= 1) {
      ...
      shadowErr = '"'.red + shadowString.red + '"'.red + ' is an invalid option for ' + property[0].underline + '.';
    }
    ...
    if (values.length >= 1) {
      ...
      var errorMessage = '"data-h2-'.red + property[0].red + '"'.red + " accepts 1 or more values, and you've specified " + values.length + ' ('.red + values.toString().red + ')'.red + '.';
