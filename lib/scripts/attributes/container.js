 ...
 */
function parseContainer(argv, property, selector, values) {
  try {
    ...
    if (values.length == 2) {
      ...
      var errorMessage = '"data-h2-'.red + property[0].red + '"'.red + ' accepts 2 values, and you\'ve specified ' + parsedValues.length + '.'
      h2Error(errorMessage);
      throw 'Error';
    }
  } catch (err) {
