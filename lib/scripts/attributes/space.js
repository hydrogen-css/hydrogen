 ...
 */
function parseSpace(argv, property, selector, values) {
  try {
    // Simple attributes only accept 1 value, so check for the array length
    var value1 = parseWhitespace(argv, property, values[0]);
    if (values.length == 1) {
      ...
      if (value1 == null) {
        ...
        var cssString = '{' + property[0] + ': ' + value1 + ';transition: all 0.2s ease;}';
      ...
      var value2 = parseWhitespace(argv, property, values[1]);
      // Check to make sure the unit was parsed as something viable
      if (
        value1 == null ||
        value2 == null) {
        ...
        // It was something viable, assemble the CSS
        var cssString = '{' + property[0] + '-top: ' + value1 + ';' + property[0] + '-bottom: ' + value1 + ';' + property[0] + '-right: ' + value2 + ';' + property[0] + '-left: ' + value2 + ';transition: all 0.2s ease;}';
        // Assemble the CSS array
        var attributeCSS = [
          selector + cssString
        ]
      ...
      var value2 = parseWhitespace(argv, property, values[1]);
      var value3 = parseWhitespace(argv, property, values[2]);
      var value4 = parseWhitespace(argv, property, values[3]);
      // Check to make sure the unit was parsed as something viable
      if (
        value1 == null ||
        value2 == null ||
        ...
        // It was something viable, assemble the CSS
        var cssString = '{' + property[0] + '-top: ' + value1 + ';' + property[0] + '-bottom: ' + value3 + ';' + property[0] + '-right: ' + value2 + ';' + property[0] + '-left: ' + value4 + ';transition: all 0.2s ease;}';
        // Assemble the CSS array
        var attributeCSS = [
          selector + cssString
        ]
