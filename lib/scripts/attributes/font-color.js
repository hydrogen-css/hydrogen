 ...
 */
function parseFontColor(property, selector, values) {
  try {
    ...
    var fontColorErr;
    if (values.length >= 1) {
      fontColor = parseColor(property, values[0]);
    ...
    if (values.length == 1) {
      if (fontColor == null) {
        ...
        if (fontColor.type == 'solid') {
          cssString = '{color: ' + fontColor.color + ';}';
        } else if (fontColor.type == 'gradient') {
          cssString = '{color: ' + fontColor.fallback + ';background-image: ' + fontColor.color + ';background-clip: text;-webkit-text-fill-color: transparent;}';
        }
      }
    } else {
