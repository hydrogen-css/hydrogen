 ...
 */
function parseVisibility(argv, property, selector, values) {
  try {
    ...
    if (values.length == 1) {
      ...
      if (visibility == 'invisible') {
        cssString = '{height: 1px;overflow: hidden;position: absolute;top: 0;left: -100vw;width: 1px;}';
      } else if (visibility == 'hidden') {
        cssString = '{display: none;visibility: hidden;}';
      } else if (visibility == 'visible') {
        cssString = '{display: block;height: auto;overflow: auto;position: static;top: auto;left: auto;width: auto;visibility: visible;}';
      } else {
        // They specified an invalid option, so log and throw an error
        var errorMessage = '"'.red + visibility.red + '"'.red + ' is an invalid option for ' + property[0].underline + '. Please use visible, hidden, or invisible.';
