...
function parse_user_settings(args) {
  try {
    ...
    if (argv.h2_verbose) {
      ...
      !user_configuration.logging ||
      (user_configuration.logging &&
        user_configuration.logging.verbose_console_output == undefined)
    ) {
      log_message({
        type: 'system',
        step: 'Validating settings...',
    ...
    let validated_user_configuration = validate_user_settings(
      argv,
      user_configuration
    );
    ...
    if (validated_user_configuration.modes) {
      ...
      if (validated_user_configuration.modes.dark) {
        if (
          validated_user_configuration.modes.dark.auto_apply_styles != undefined
        ) {
    ...
    if (validated_user_configuration.processing) {
      if (
        validated_user_configuration.processing.include_reset_css != undefined
      ) {
        processing_config.include_reset_css =
          validated_user_configuration.processing.include_reset_css;
      }
      if (
        validated_user_configuration.processing.browser_prefix_css != undefined
      ) {
      ...
      if (validated_user_configuration.processing.minify_css != undefined) {
        processing_config.minify_css =
          validated_user_configuration.processing.minify_css;
      }
      if (
    ...
    if (validated_user_configuration.logging) {
      if (validated_user_configuration.logging.generate_logs != undefined) {
        logging_config.generate_logs =
          validated_user_configuration.logging.generate_logs;
      }
      if (validated_user_configuration.logging.show_timers != undefined) {
        logging_config.show_timers =
          validated_user_configuration.logging.show_timers;
      }
      if (
        validated_user_configuration.logging.verbose_console_output != undefined
      ) {
          ...
          theme.colors.forEach(function (color_setting) {
            ...
            modes.forEach(function (mode) {
              ...
              if (
                ...
                validated_user_configuration.modes.dark
