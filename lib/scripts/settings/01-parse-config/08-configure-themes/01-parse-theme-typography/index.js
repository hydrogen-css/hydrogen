...
// Local functions
const { parse_type_scale_item } = require('./01-parse-type-scale-item');
...
function parse_theme_typography(config, type_config) {
  try {
    ...
    // Check for the line_heights configuration and whether it includes copy value, and if not, check for the deprecated line_height value and use that, then override Hydrogen's default if a value is found
    if (type_config.line_heights && type_config.line_heights.copy) {
      ...
      } else if (typeof type_config.line_heights.copy === 'string') {
      ...
      } else if (typeof type_config.line_height === 'string') {
    ...
    // Calculate and set the copy/paragraph scales
    let copy_size = 1;
    let default_copy_lh = 'var(--h2-base-line-height)';
    let copy_lh = parse_type_scale_item(type_config, 'copy', default_copy_lh);
    type_config.copy = {
      size: copy_size + 'rem',
      line_height: copy_lh,
    };
