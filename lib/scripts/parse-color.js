...
function parseColor(property, colorString) {
  try {
    ...
    if (colorString.includes('.')) {
      var error = '"data-h2-'.red + property[0].red + '"'.red + ' is using a color (' + colorString.red + ") that contains an invalid dot. If you were trying to use a modifier on a configured color, check to make sure you're using the correct key.";
      h2Error(error);
      return null;
    } else {
      var finalColor = {
