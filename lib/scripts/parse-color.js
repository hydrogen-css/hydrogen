  ...
  try {
      ...
      if (colorKey[0] == color.key) {
        ...
        if (scaleModifier != null) {
          if (scaleModifier == 'light') {
            ...
            console.log('⛔ [' + 'Hydrogen'.magenta + ']', '"'.red + scaleModifier.red + '"'.red, 'is not a proper scale modifier for a color. Pleasee use light, lighter, lightest, dark, darker, or darkest.');
            return null;
          }
          if (opacityModifier != null) {
    ...
    for (const [key, value] of Object.entries(gradientMap)) {
      if (workingColor == key) {
        ...
        console.log('⛔ [' + 'Hydrogen'.magenta + ']', 'Gradients are not supported for the', property.underline, 'attribute.');
        return null;
      }
    }
