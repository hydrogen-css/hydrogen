  ...
  try {
        ...
        if (scaleModifier != null) {
          if (scaleModifier == 'light') {
            if (color.scale.light == 'auto' || color.scale.light == null || color.scale.light == undefined) {
              ...
              // workingColor = Color(actualColor).saturate(0.2).mix(Color('white'), 0.2);
            ...
            if (color.scale.lighter == 'auto' || color.scale.lighter == null || color.scale.lighter == undefined) {
              ...
              // workingColor = Color(actualColor).saturate(0.4).mix(Color('white'), 0.4);
              workingColor = Color(workingColor).rgb().string();
            } else {
            ...
            if (color.scale.lightest == 'auto' || color.scale.lightest == null || color.scale.lightest == undefined) {
              ...
              // workingColor = Color(actualColor).saturate(0.6).mix(Color('white'), 0.6);
              workingColor = Color(workingColor).rgb().string();
            } else {
            ...
            if (color.scale.dark == 'auto' || color.scale.dark == null || color.scale.dark == undefined) {
              // workingColor = Color(actualColor).mix(Color(actualColor).rotate(-20).mix(Color('black'), 0.25), 0.25);
              workingColor = Color(actualColor).saturate(0.2).mix(Color('black'), 0.25);
              // workingColor = darkenBy(Color(actualColor), 0.15);
              workingColor = Color(workingColor).rgb().string();
            } else {
            ...
            if (color.scale.darker == 'auto' || color.scale.darker == null || color.scale.darker == undefined) {
              // workingColor = Color(actualColor).mix(Color(actualColor).rotate(-20).mix(Color('black'), 0.5), 0.5);
              workingColor = Color(actualColor).saturate(0.2).mix(Color('black'), 0.5);
              // workingColor = darkenBy(Color(actualColor), 0.3);
              workingColor = Color(workingColor).rgb().string();
            } else {
            ...
            if (color.scale.darkest == 'auto' || color.scale.darkest == null || color.scale.darkest == undefined) {
              // workingColor = Color(actualColor).mix(Color(actualColor).rotate(-20).mix(Color('black'), 0.75), 0.75);
              workingColor = Color(actualColor).saturate(0.2).mix(Color('black'), 0.75);
              // workingColor = darkenBy(Color(actualColor), 0.45);
              workingColor = Color(workingColor).rgb().string();
            } else {
              workingColor = color.scale.darkest;
            }
          } else {
            var error = '"'.red + scaleModifier.red + '"'.red + 'is not a proper scale modifier for a color. Please use light, lighter, lightest, dark, darker, or darkest.';
            h2Error(error);
            return null;
          }
          if (opacityModifier != null) {
            workingColor = Color(workingColor).alpha(opacityModifier).rgb().string();
          }
        } else {
          if (opacityModifier != null) {
    ...
    if (property == 'bg-color' || property == 'overlay' || property == 'font-color' || property == 'color') {
      if (settings.gradients == null) {
        ...
        for (const gradientSetting of userGradients) {
          if (workingColor == gradientSetting.key) {
            workingColor = gradientSetting.gradient;
            var fallbackColor = gradientSetting.fallback;
            var finalColor = {
              type: 'gradient',
              color: workingColor,
    ...
    if (uneditedColor == workingColor) {
      if (opacityModifier != null) {
        workingColor = Color(colorKey[0]).alpha(opacityModifier).rgb().string();
      }
      // Return the color and type
      var finalColor = {
        type: 'solid',
