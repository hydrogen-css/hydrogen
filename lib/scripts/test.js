...
function getLineHeight(settings, fontSize) {
  try {
...
function getColor (settings, gradientMap, property, colorString) {
  try {
...
function assembleCoreCSS(settings) {
  try {
    var coreCSSString = '';
    ...
    var h6LineHeight = getLineHeight(settings, h6Size);
    var h5Size = h6Size * fontScale;
    var h5Unit = h5Size + 'rem';
    var h5LineHeight = getLineHeight(settings, h5Size);
    var h4Size = h5Size * fontScale;
    var h4Unit = h4Size + 'rem';
    var h4LineHeight = getLineHeight(settings, h4Size);
    var h3Size = h4Size * fontScale;
    var h3Unit = h3Size + 'rem';
    var h3LineHeight = getLineHeight(settings, h3Size);
    var h2Size = h3Size * fontScale;
    var h2Unit = h2Size + 'rem';
    var h2LineHeight = getLineHeight(settings, h2Size);
    var h1Size = h2Size * fontScale;
    var h1Unit = h1Size + 'rem';
    var h1LineHeight = getLineHeight(settings, h1Size);
...
function buildCSS() {
  try {
    console.time(
      'Hydrogen ' + '[TIMELOG]'.magenta + ' Code scrape and CSS contruction time was',
      'buildTime'
    );
    var hydrogenAttributes = scrapeFiles();
    var settings = loadHydrogenSettings();
    var hydrogen = assembleCoreCSS(settings);
    ...
    hydrogenAttributes.forEach(function(attribute) {
      ...
      values.forEach(function(value) {
        ...
        if (property == 'custom') {
          ...
          var finalColor = getColor(settings, gradientMap, property, parsedValues[0]);
          ...
          if (lineHeight == null || lineHeight == undefined) {
            lineHeight = getLineHeight(settings, numericFontSize);
          }
          // Assemble CSS
...
function processCSS() {
  try {
    console.time(
      'Hydrogen ' + '[TIMELOG]'.magenta + ' Autoprefixer and CSSNano process time was',
      'processTime'
    );
