        ...
        values.forEach(function(value) {
          ...
          if (property == 'custom') {
            ...
            if (parsedValues.length == 1 || parsedValues.length == 3) {
              ...
              var borderWidth = parseWhitespace(argv, property, parsedValues[1]);
              // Get the color or gradient
              ...
              if (borderWidth == null) {
                ...
                if (borderColor == null) {
                  var errorMessage = '"'.red + parsedValues[3].red + '"'.red + ' is an invalid color for ' + 'border'.underline + '.'
                  h2Error(errorMessage);
                  falseAttribute = true;
                } else {
                  if (borderColor.type == 'solid') {
                    ...
                    var errorMessage = 'gradients'.red + ' are not supported by ' + 'border'.underline + '.'
                    h2Error(errorMessage);
                    falseAttribute = true;
                  }
                }
              ...
              // 4 or more values are not possible for borders
              var errorMessage = '"data-h2-'.red + property[0].red + '"'.red + ' requires 2 or 4 values, and you\'ve specified ' + parsedValues.length + '.'
              h2Error(errorMessage);
              falseAttribute = true;
            }
            ...
            // Height only accepts 1 value that should be a space unit, and therefore should be parsed for multipliers
            if (parsedValues.length == 1) {
              var height = parseWhitespace(argv, property, parsedValues[0]);
              ...
              if (height == null) {
                ...
                cssString = '{height: ' + height + ';transition: 0.2s ease all;}';
              }
            } else {
            ...
            // Width only accepts 1 value that should be a space unit, and therefore should be parsed for multipliers
            if (parsedValues.length == 1) {
              var width = parseWhitespace(argv, property, parsedValues[0]);
              // Check to make sure the unit was parsed as something viable
              if (width == null) {
                // It was a bad unit value, trigger an error
                falseAttribute = true;
                ...
                // It was something viable, assemble the CSS
                cssString = '{width: ' + width + ';transition: 0.2s ease all;}';
              }
            } else {
