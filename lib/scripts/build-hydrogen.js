  ...
  try {
    ...
    if (validation === true) {
      ...
      if (debug) {
        ...
        fs.writeFileSync(path.join(getUserOutput('string') + '/hydrogen.logs.attributes.json'), JSON.stringify(tempAttributes));
      }
      // Build the core CSS
        ...
        if (values != null && values.length != 0 && values[0].length != 0) {
          values.forEach(function (value) {
            ...
            if (lintParens(value) == false) {
              var error = '"'.red + property[0].red + '"'.red + ' contains a query (' + value.red + ') that appears to have an extra parenthesis. This might be a typo.';
              h2Error(error);
              falseAttribute = true;
            } else {
              ...
              if (finalMode != 'dark') {
                ...
                var cleanedDarkCheckValues = JSON.parse(JSON.stringify(values));
                cleanedDarkCheckValues = cleanedDarkCheckValues.filter((item) => item !== value);
                ...
                if (cleanedDarkCheckValues != null && cleanedDarkCheckValues.length != 0 && cleanedDarkCheckValues[0].length != 0) {
                  cleanedDarkCheckValues.forEach(function (checkValue) {
                    ...
                    newDebugValue.cleanedValues = newDebugValue.cleanedValues.concat(cleanedValue);
              ...
              newDebugValueSet.values = newDebugValueSet.values.concat(newDebugValue);
              ...
              if (internalValues != null && internalValues.length != 0 && internalValues[0].length != 0) {
                ...
                if (property == 'custom') {
                  ...
                  valueCSS = parseSimpleAttribute(property, cssSelector, parsedValues);
                  if (valueCSS == null) {
                    falseAttribute = true;
                  }
                  ...
                  valueCSS = parseBackgroundColor(property, cssSelector, parsedValues);
                  if (valueCSS == null) {
                    falseAttribute = true;
                  }
                } else if (property == 'border') {
                  valueCSS = parseBorder(property, mediaQuery, cssSelector, parsedValues);
                  if (valueCSS == null) {
                    falseAttribute = true;
                  }
                } else if (property == 'container') {
                  valueCSS = parseContainer(property, cssSelector, parsedValues);
                  if (valueCSS == null) {
                    falseAttribute = true;
                  }
                  ...
                  valueCSS = parseFlexGrid(property, cssSelector, parsedValues);
                  if (valueCSS == null) {
                    falseAttribute = true;
                  }
                } else if (property == 'flex-item') {
                  valueCSS = parseFlexItem(property, cssSelector, parsedValues);
                  if (valueCSS == null) {
                    falseAttribute = true;
                  }
                  ...
                  valueCSS = parseFontColor(property, cssSelector, parsedValues);
                  if (valueCSS == null) {
                    falseAttribute = true;
                  }
                } else if (property == 'font-family') {
                  valueCSS = parseFontFamily(property, cssSelector, parsedValues);
                  if (valueCSS == null) {
                    falseAttribute = true;
                  }
                } else if (property == 'font-size') {
                  valueCSS = parseFontSize(property, cssSelector, parsedValues);
                  if (valueCSS == null) {
                    falseAttribute = true;
                  }
                  ...
                  valueCSS = parseGridTemplate(property, cssSelector, parsedValues);
                  if (valueCSS == null) {
                    falseAttribute = true;
                  }
                  ...
                  valueCSS = parseOverflow(property, cssSelector, parsedValues);
                  if (valueCSS == null) {
                    falseAttribute = true;
                  }
                } else if (property == 'overlay') {
                  valueCSS = parseOverlay(property, cssSelector, parsedValues);
                  if (valueCSS == null) {
                    falseAttribute = true;
                  }
                  ...
                  valueCSS = parsePosition(property, cssSelector, parsedValues);
                  if (valueCSS == null) {
                    falseAttribute = true;
                  }
                  ...
                  valueCSS = parseTransition(property, cssSelector, parsedValues);
                  if (valueCSS == null) {
                    falseAttribute = true;
                  }
                  ...
                  valueCSS = parseVisibility(property, cssSelector, parsedValues);
                  if (valueCSS == null) {
                    falseAttribute = true;
                  }
      ...
      if (debug) {
        fs.writeFileSync(getUserOutput('string') + '/hydrogen.logs.values.json', JSON.stringify(debugValueArray));
      }
      ...
      if (debug) {
        ...
        fs.writeFileSync(path.join(getUserOutput('string') + '/hydrogen.logs.media.json'), JSON.stringify(mediaQueryArray));
      }
      // Delete any existing raw CSS files
      if (fs.existsSync(path.join(getUserOutput('string') + '/hydrogen.raw.css')) == true) {
        fs.unlinkSync(path.join(getUserOutput('string') + '/hydrogen.raw.css'));
      }
      // Write the new file
      fs.writeFileSync(path.join(getUserOutput('string') + '/hydrogen.raw.css'), hydrogen);
      ...
      if (processStatus === true) {
        ...
        if (debug) {
          console.log('🔔 [' + 'Hydrogen'.magenta + ']', "You've set Hydrogen to generate output logs and retain a pre-processed CSS file in " + path.resolve(getUserOutput('string')).green + ' for debugging purposes.');
        }
        // Log success
        console.log('✅ [' + 'Hydrogen'.magenta + ']', 'A CSS file was successfully built in ' + path.resolve(getUserOutput('string')).green);
      }
    }
  } catch (error) {
    h2Error(error);
