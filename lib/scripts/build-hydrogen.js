      ...
      if (values != null) {
        values.forEach(function(value) {
          ...
          if (property == 'custom') {
            ...
            // Produce CSS string
            cssString = '{align-items: ' + itemAlignment + ';display: flex;flex-wrap: wrap; --gridGap: ' + itemGap + ';gap: var(--gridGap);' + wrapperPadding + '}';
            // Add the final CSS to the output file
            valueCSS = [
              cssSelector + cssString
            ];
            ...
            if (parsedValues[0] == 'auto') {
              ...
              if (firstColumn != null && secondColumn != null) {
                cssString = '{flex: 0 0 calc((' + firstColumn + ' / ' + secondColumn + ' * 100%) - (((' + secondColumn + ' - 1) * var(--gridGap)) / ' + secondColumn + '));max-width: calc(' + firstColumn + ' / ' + secondColumn + ' * 100%);min-width: 0;}';
              } else {
                console.error(
                  'Hydrogen',
                  '[ERROR]'.red,
