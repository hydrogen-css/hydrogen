...
function lintParens(property, query) {
  var brackets = 0;
  ...
  try {
    ...
    if (validation === true) {
      ...
      if (debug) {
        ...
        fs.writeFileSync(getUserOutput(argv, 'string') + '/hydrogen.logs.attributes.json', JSON.stringify(tempAttributes));
      }
      // Build the core CSS
      ...
      hydrogenAttributes.forEach(function (attribute) {
        ...
        if (values != null && values.length != 0 && values[0].length != 0) {
          values.forEach(function (value) {
            ...
            if (lintParens(property, value) == false) {
      ...
      if (debug) {
        fs.writeFileSync(getUserOutput(argv, 'string') + '/hydrogen.logs.values.json', JSON.stringify(debugValueArray));
      }
      ...
      if (debug) {
        ...
        fs.writeFileSync(getUserOutput(argv, 'string') + '/hydrogen.logs.media.json', JSON.stringify(mediaQueryArray));
      }
      // Delete any existing raw CSS files
      if (fs.existsSync(getUserOutput(argv, 'string') + '/hydrogen.raw.css') == true) {
        fs.unlinkSync(getUserOutput(argv, 'string') + '/hydrogen.raw.css');
      }
      // Write the new file
      ...
      var processStatus = processCSS(argv);
      if (processStatus === true) {
        ...
        if (debug) {
          console.log('🔔 [' + 'Hydrogen'.magenta + ']', "You've set Hydrogen to generate output logs and retain a pre-processed CSS file in " + getUserOutput(argv, 'string').green + '/'.green + ' for debugging purposes.');
        }
        // Log success
