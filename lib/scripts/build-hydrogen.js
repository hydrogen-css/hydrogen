...
const { clean_logs } = require('./logs/log-clean');
      ...
      if (user_media_array) {
        ...
        if (validation) {
          ...
          if (settings.build.logs === true) {
            if (
              !fs.existsSync(settings.runtime.output.string + '/hydrogen-logs')
            ) {
          ...
          if (hydrogen) {
            ...
            let parsed_attribute_data =
              parse_input_data_for_attributes(settings);
            ...
            if (parsed_attribute_data) {
              ...
              if (
                fs.existsSync(
                  path.join(
                    settings.runtime.output.string + '/hydrogen.raw.css'
                  )
                ) == true
              ) {
                fs.unlinkSync(
              ...
              if (settings.build.logs === true) {
                fs.writeFileSync(
                  path.join(
                    settings.runtime.logging.directory + '/hydrogen.raw.css'
                  ),
                  hydrogen
                );
              }
