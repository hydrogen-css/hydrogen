...
function buildHydrogen(argv) {
  try {
    ...
    console.time('⌚ [' + 'Hydrogen'.magenta + '] ' + 'Total Hydrogen build time', 'totalTime');
    ...
    console.time('⌚ [' + 'Hydrogen'.magenta + '] ' + 'CSS contruction time', 'buildTime');
        ...
        values.forEach(function (value) {
          ...
          if (property == 'custom') {
            ...
            if (finalColor == null || finalColor == undefined) {
              console.error('⛔ [' + 'Hydrogen'.magenta + ']', '"'.red + parsedValues[0].red + '"'.red, 'is an invalid color for ' + 'font-color'.underline + '.');
              falseAttribute = true;
            } else {
            ...
            if (fontSize == 'h1') {
              ...
              if (fontSize.match(/(rem)|(px)/g) != null) {
                ...
                console.error('⛔ [' + 'Hydrogen'.magenta + ']', '"'.red + fontSize.red + '"'.red, 'is an invalid option for ' + 'font-size'.underline + '. Please use one of the font scale values, a px, or a rem value.');
                falseAttribute = true;
              }
            }
            ...
            valueCSS = ['[data-h2-position]' + cssSelector + ',', '[data-h2-visibility]' + cssSelector + ',', cssSelector + cssString];
            ...
            if (orientation == 'all' || orientation == 'both') {
              ...
              console.error('⛔ [' + 'Hydrogen'.magenta + ']', '"'.red + orientation.red + '"'.red, 'is an invalid orientation option for ' + 'overflow'.underline + '. Please use all, x, or y.');
              falseAttribute = true;
            }
            // Add to media css string, and only assemble the hydrogen file at the end, with the correct order for queries by looping through them
            // Add the final CSS to the output file
            ...
            // Log an error if the color comes back wrong, otherwise build the CSS
            if (finalColor == null || finalColor == undefined) {
              console.error('⛔ [' + 'Hydrogen'.magenta + ']', '"'.red + parsedValues[0].red + '"'.red, 'is an invalid color for ' + 'overlays'.underline + '.');
              falseAttribute = true;
            } else {
              if (finalColor.type == 'solid') {
            ...
            // Add the final CSS to the output file
            var valueCSS = [cssSelector + cssString, cssSelector + ' > * {position: relative;}'];
            ...
            // Add the final CSS to the output file
            valueCSS = ['[data-h2-visibility]' + cssSelector + ',', cssSelector + cssString];
    ...
    fs.writeFileSync(envSrc + settings.output + '/hydrogen.raw.css', hydrogen, function (err) {
      if (err) {
        h2Error(err);
      }
    });
    console.timeEnd('⌚ [' + 'Hydrogen'.magenta + '] ' + 'CSS contruction time', 'buildTime');
  } catch (err) {
    console.log(err);
    return false;
  }
