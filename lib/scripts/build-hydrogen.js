...
function buildHydrogen() {
  try {
    console.time(
      'Hydrogen ' + '[LOGGING]'.magenta + ' Code scrape and CSS contruction time was',
      'buildTime'
    );
    var settings = loadSettings();
    var hydrogenAttributes = parseAttributes();
    var hydrogen = generateCoreCSS();
    var mediaQueryArray = generateMediaQueryArray();
    ...
    hydrogenAttributes.forEach(function(attribute) {
      ...
      values.forEach(function(value) {
        ...
        if (property == 'custom') {
          ...
          var finalColor = parseColor(property, parsedValues[0]);
          ...
          if (lineHeight == null || lineHeight == undefined) {
            lineHeight = calculateLineHeight(numericFontSize);
          }
          // Assemble CSS
