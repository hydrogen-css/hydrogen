...
var { parse_settings } = require('./settings-parser');
var { validate_settings } = require('./settings-validation');
var { getUserOutput } = require('./generate-paths');
var { clean_logs } = require('./log-clean');
var { date_time, log_info } = require('./logs');
...
function h2_compile_hydrogen(command, args) {
  try {
    ...
    if (validation === true) {
      ...
      if (
        fs.existsSync(
          path.join(getUserOutput(settings, 'string') + '/hydrogen.raw.css')
        ) == true
      ) {
        fs.unlinkSync(
          path.join(getUserOutput(settings, 'string') + '/hydrogen.raw.css')
    ...
    // Log any errors that weren't accounted for ===============================
    log_info('error', 'Unknown error', 'Build', null, null, null, null, error);
    // Return false
    return false;
  }
}
