...
function getQueries(attribute, property) {
  ...
  attribute = attribute.slice(0, -1); // Removes the final trailing character, assumed to be a " or '
  var result = [];
  var open_bracket = 0;
  var curr_attribute = '';
...
function h2_compile_hydrogen() {
  try {
    ...
    if (validation === true) {
      ...
      var hydrogen_attribute_data = parse_input_data_for_attributes(debug);
        ...
        fs.writeFileSync(
          path.join(
            getUserOutput('string') +
              '/hydrogen-logs/hydrogen.logs.attributes.' +
              file_slug +
              '.json'
          ),
          JSON.stringify(tempAttributes, null, ' ')
        );
      }
      // Build the core CSS
          ...
          values.forEach(function (value) {
            ...
            if (lintParens(value) == false) {
              h2_error_detail(
                'parens',
                attribute,
                'C:\\Users\\myUser\\myCode\\input\\index.html'
              );
              falseAttribute = true;
            } else {
      ...
      if (debug) {
        ...
        fs.writeFileSync(
          getUserOutput('string') +
            '/hydrogen-logs/hydrogen.logs.values.' +
            file_slug +
            '.json',
      ...
      if (debug) {
        ...
        fs.writeFileSync(
          path.join(
            getUserOutput('string') +
              '/hydrogen-logs/hydrogen.logs.media.' +
              file_slug +
              '.json'
          ),
      ...
      if (processStatus === true) {
        ...
        if (debug) {
          h2_warning_detail('logs', null, path.join(getUserOutput('string')));
        }
        // Log success
        console.log(
          '✅ [' + 'Hydrogen'.magenta + ']',
