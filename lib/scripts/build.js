...
function loadHydrogenConfig(done) {
  ...
  if (fs.existsSync('./hydrogen.config.json') == false) {
    console.log('[ERROR] Hydrogen: you do not have a hydrogen.config.json file in your project root. Please create one to continue.'.red);
    return false;
  } else {
...
function getUserMarkup() {
  return src('./' + config.folders.markup + '/**/*')
