 ...
 * @param {"true" | "false" | "warning"} [args.success]
...
function log_message(args) {
  try {
    ...
    if (args.times) {
      ...
      time_message = colors.dim(' (') + colors.dim(ms.toString()) + colors.dim('ms)');
    }
    // Build the build step message if requested
    let step_message = '';
    if (args.step) {
      step_message = colors.dim(' - ') + highlight(args.step) + time_message;
    }
    let lightning_line = '';
    if (args.error && args.error.loc) {
      let line_value = '';
      let column_value = '';
      if (args.error.loc.start) {
        line_value = args.error.loc.start.line;
        column_value = args.error.loc.start.column;
      } else if (args.error.loc.line) {
        line_value = args.error.loc.line;
        column_value = args.error.loc.column;
      }
      lightning_line =
        '\n\n' +
        colors.dim('  Location > ') +
        wrap_message(
          'Line: ' +
            ...
            column_value +
            '\n' +
            'This location can be found by enabling logs in your Hydrogen configuration file and then checking the output inside of hydrogen.raw.css.'
        );
    }
    ...
    if (args.message) {
      let prefix = '\n\n' + colors.dim('  Message  > ');
      let body = wrap_message(args.message);
      if (args.success && args.success === 'true') {
        body = colors.green(body);
      }
      if (args.success && args.success === 'false') {
        body = colors.red(body);
      }
      if (args.success && args.success === 'warning') {
        body = colors.yellow(body);
      }
      context_message = prefix + body;
    }
