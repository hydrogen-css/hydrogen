...
function log_message(options) {
  try {
    ...
    let step_message = '';
    let step_newline = '';
    if (options.step && !options.message) {
      step_newline = '\n';
    }
    if (options.step) {
      step_message = ' - '.dim + highlight(options.step) + step_newline;
    }
    ...
    if (options.message) {
      context_message =
        '\n' + '             '.dim + fit_message(options.message) + '\n';
    }
    ...
    if (options.settings && options.type && options.type === 'error') {
      options.settings.logging.errors.count =
        options.settings.logging.errors.count + 1;
    } else if (options.settings && options.type && options.type === 'warning') {
      options.settings.logging.warnings.count =
        options.settings.logging.warnings.count + 1;
    }
