    ...
    if (color.opacities != null && color.opacities != undefined && color.opacities != "[]") {
      ...
      for (let opacity in color.opacities) {
        ...
        if (color.opacities[opacity].includes('%')) {
          ...
          colorOpacityString = colorOpacityString + '"' + color.key + '[' + color.opacities[opacity] + ']": rgba(' + color.color + ',' + decimal + '),';
          colorOpacityString = colorOpacityString + '"[light]' + color.key + '[' + color.opacities[opacity] + ']": color.scale(rgba(' + color.color + ', ' + decimal + '), $lightness: 25%),';
          colorOpacityString = colorOpacityString + '"[dark]' + color.key + '[' + color.opacities[opacity] + ']": color.scale(rgba(' + color.color + ', ' + decimal + '), $lightness: -15%, $saturation: -10%),';
        } else {
          colorOpacityString = colorOpacityString + '"' + color.key + '[' + color.opacities[opacity] + ']": rgba(' + color.color + ', ' + color.opacities[opacity] + '),';
          colorOpacityString = colorOpacityString + '"[light]' + color.key + '[' + color.opacities[opacity] + ']": color.scale(rgba(' + color.color + ', ' + color.opacities[opacity] + '), $lightness: 25%),';
          colorOpacityString = colorOpacityString + '"[dark]' + color.key + '[' + color.opacities[opacity] + ']": color.scale(rgba(' + color.color + ', ' + color.opacities[opacity] + '), $lightness: -15%, $saturation: -10%),';
        }
      }
