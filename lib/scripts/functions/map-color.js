    ...
    if (color.opacities != null && color.opacities != undefined && color.opacities != "[]") {
      ...
      for (let opacity in color.opacities) {
        ...
        if (color.opacities[opacity].includes('%')) {
          ...
          colorOpacityString = colorOpacityString + '"[light]' + color.key + '[' + color.opacities[opacity] + ']": color.scale(color.adjust(' + color.color + ', $alpha: ' + decimal + '), $lightness: 25%),';
          colorOpacityString = colorOpacityString + '"[dark]' + color.key + '[' + color.opacities[opacity] + ']": color.scale(color.adjust(' + color.color + ', $alpha: ' + decimal + '), $lightness: -15%, $saturation: -10%),';
        } else {
          ...
          colorOpacityString = colorOpacityString + '"[light]' + color.key + '[' + color.opacities[opacity] + ']": color.scale(color.adjust(' + color.color + ', $alpha: ' + color.opacities[opacity] + '), $lightness: 25%),';
          colorOpacityString = colorOpacityString + '"[dark]' + color.key + '[' + color.opacities[opacity] + ']": color.scale(color.adjust(' + color.color + ', $alpha: ' + color.opacities[opacity] + '), $lightness: -15%, $saturation: -10%),';
        }
      }
