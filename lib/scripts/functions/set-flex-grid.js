...
async function moveFlexgridCore(env) {
  var defaults = loadH2Defaults(env);
  var config = loadH2Config(env);
...
async function enableFlexgridCore(env) {
  var defaults = loadH2Defaults(env);
  var config = loadH2Config(env);
  var flexgridEnabled = setFlexgridConfig(env);
  if (flexgridEnabled == true) {
    if (env == "dev") {
      return src('./lib/stage/' + config.folders.styles + '/hydrogen/core.scss')
...
async function moveFlexgrid(env) {
  var defaults = loadH2Defaults(env);
  var config = loadH2Config(env);
  var flexgridEnabled = setFlexgridConfig(env);
...
async function enableFlexgrid(env, task) {
  var defaults = loadH2Defaults(env);
  var config = loadH2Config(env);
  var path;
  if (env == 'dev') {
