...
async function buildDevCSS(env, task) {
  ...
  var compiledHydrogenUtilityCSS;
  if (task == "build") {
    // Get the temporary compiled Hydrogen file, located in the user's specified location in the config.
    compiledHydrogenUtilityCSS = fs.readFileSync(path + config.folders.styles + '/hydrogen/compiled/utility.css').toString();
  } else if (task == "compile") {
    compiledHydrogenUtilityCSS = fs.readFileSync(path + config.folders.styles + '/hydrogen/compiled/compile.css').toString();
  }
  // Assemble the string.
  var devHydrogen = fontFaceCSS + compiledHydrogenCoreCSS + compiledHydrogenUtilityCSS;
  // Write the file.
  fs.writeFile(path + config.folders.styles + '/hydrogen.css', devHydrogen, function(err) {
    if (err) {
