...
function loadH2Defaults(env) {
  var defaults;
  if (env == 'dev') {
    defaults = JSON.parse(fs.readFileSync(path.resolve(__dirname, '../../hydrogen.default.json')));
  } else if (env == 'prod') {
    defaults = JSON.parse(
      fs.readFileSync(
...
function getH2DestinationPath(env) {
  try {
    ...
    if (env != '' && env != null && env != undefined) {
      if (env == 'dev') {
        projectPath = projectPath + './lib/stage/';
        return projectPath;
      } else if (env == 'prod') {
...
function getH2OutputDirectory(env) {
  try {
    ...
    var outputDir = '';
    if (config.output != null && config.output != undefined) {
      ...
      config.folders.styles != null &&
      config.folders.styles != undefined
    ) {
