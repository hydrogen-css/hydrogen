...
function createShadowSassVariables(env) {
  ...
  shadowConfig.forEach(function (shadows) {
    var shadowVariable =
      '$h2-shadow-' + shadows.key + ': ' + shadows.value + ';\n';
...
function loadShadowMap(env) {
  ...
  // Assemble the map.
  var shadowMap =
    shadowMapStringStart + shadowMapStringContent + shadowMapStringEnd;
  // Return the map.
  return shadowMap;
}
...
function setShadowMap(env) {
  ...
  if (env == 'dev') {
    ...
    return src(
      './node_modules/@hydrogen-design-system/hydrogen.css/lib/styles/maps/_map-shadow.scss'
    )
      .pipe(footer(shadowMap))
