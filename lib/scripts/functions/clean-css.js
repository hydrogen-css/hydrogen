...
async function createCleanCSS(env) {
  ...
  var stateMap =
    '$h2-map-states: ("": "",":a": ":active",":h": ":hover",":f": ":focus",":d": ":disabled");';
  ...
  if (
    ...
    config.media.forEach(function (mediaQuery) {
      ...
      queryKeys = queryKeys + mediaQuery.key;
    });
  } else {
  ...
  // console.log('queries: ', queries, 'keys: ', queryKeys);
  // Get individual attribute values (x(y)).
  // var valueRegex = /([^"' ]*?)\(([^)]*)\)/g;
  var valueRegexString = '[' + queryKeys + ']\\(([^)]*)\\)';
  var valueRegex = new RegExp(valueRegexString, 'g');
  ...
  // console.log(markup);
  for (let item in config.folders.markup) {
    fs.readdirSync(path + config.folders.markup[item]).forEach((file) => {
      // console.log(file);
      var read = fs
        .readFileSync(path + config.folders.markup[item] + '/' + file)
        .toString();
      fullUsedAttributes = fullUsedAttributes.concat(read.match(dataRegex));
      // console.log(config.folders.markup[item], read.match(dataRegex));
    });
  }
    ...
    usedAttributes.forEach(function (attribute) {
      ...
      if (values != null) {
        values.forEach(function (value) {
          ...
          var mediaValueString = '^[' + queryKeys + '][^:(]*';
          var mediaValueRegex = new RegExp(mediaValueString, 'g');
          var mediaValue = value.match(mediaValueRegex);
          ...
          if (queryMatch != null) {
            ...
            if (
              ...
              utilityCSS = fs
                ...
                .toString();
              var cssMatch = utilityCSS.match(cssRegex); // Returns the full CSS selector: [data-h2-ATTRIBUTE*="MEDIA(VALUE)"]***{CSS}
