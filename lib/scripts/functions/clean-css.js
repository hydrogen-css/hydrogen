...
async function createCleanCSS(env) {
  console.time('H2 ' + '[TIMELOG]'.magenta + ' Code scrape and CSS build time was', 'cleanTime');
  ...
  // console.time('H2 ' + '[TIMELOG]'.magenta + ' Attribute search time was', 'searchTime');
  var fullUsedAttributes = markup.match(dataRegex);
  // console.timeEnd('H2 ' + '[TIMELOG]'.magenta + ' Attribute search time was', 'searchTime');
  ...
  if (usedAttributes != null) {
      ...
      if (values != null) {
        values.forEach(function(value) {
          ...
          if (queryMatch != null) {
            ...
            var getUtility;
            if (utility[0] == 'data-h2-flex-grid' || utility[0] == 'data-h2-flex-item') {
              getUtility = 'flex-grid';
            } else {
              getUtility = utility[0].replace('data-h2-', '');
            }
            var utilityCSS
            if (fs.existsSync(path + config.folders.styles + '/hydrogen/compressed/' + getUtility + '.css') == false) {
              if (getUtility == 'flex-grid') {
                if (flexgridWarning == false) {
                  console.log('H2', '[ERROR]'.red, 'It looks like you\'re using flex-grid in your code, but it hasn\'t been enabled in your config file.');
                  flexgridWarning = true;
                } else {
                  // Do nothing.
                }
              } else {
                console.log('H2', '[WARNING]'.yellow, 'There is an incorrect Hydrogen attribute', '('.bold + utility[0].bold + ')'.bold, 'in your code.');
              }
            } else {
              utilityCSS = fs.readFileSync(path + config.folders.styles + '/hydrogen/compressed/' + getUtility + '.css').toString();
              ...
              if (cssMatch != null) {
                ...
                var opacityMatch = mediaVariable.match(/\[([\d]*[.].*|[\d]*[%])\]/g);
                if (opacityMatch != null && opacityMatch != false) {
                  var brokenUtility = utility[0] + '="' + value + '"';
                  console.log('H2', '[WARNING]'.yellow, 'There seems to be a Hydrogen attribute that\'s trying to use an opacity modifier that doesn\'t match the modifiers set in your configuration file. Double check that you\'ve enabled the correct opacity value for this color:', brokenUtility.bold);
                }
              }
            }
          } else {
            if (config.mediaWarnings != false) {
              console.log('H2', '[WARNING]'.yellow, 'There is a media query being used in your code', '('.bold + mediaValue[0].bold + ')'.bold, 'that hasn\'t been defined in the media section of your configuration file.');
            }
          }
        }); 
      }
    ...
    if (fs.existsSync(path + config.folders.styles + '/hydrogen/cleaned') == true) {
      fs.rmdirSync(path + config.folders.styles + '/hydrogen/cleaned', { recursive: true });
    }
    ...
    console.error('H2', '[ERROR]'.red, 'We couldn\'t find any Hydrogen attributes in your code so the build failed.');
    await Promise.reject(new Error('Try adding some attributes to your code!'));
  }
  console.timeEnd('H2 ' + '[TIMELOG]'.magenta + ' Code scrape and CSS build time was', 'cleanTime');
