...
function createFontScaleSassVariables(env) {
  ...
  var fontScaleCaptionString = '$h2-font-size-caption: ' + fontScaleCaption + 'rem;\n';
  ...
  fontScaleVariables = fontScaleVariables + fontScaleH1String + fontScaleH2String + fontScaleH3String + fontScaleH4String + fontScaleH5String + fontScaleH6String + fontScaleBaseString + fontScaleCaptionString;
  ...
  if (config.fonts != null && config.fonts != undefined && config.fonts.length > 0) {
    ...
    fontFamilyConfig.forEach(function(fontFamily) {
      if (fontFamily.loadType == 'font-face' || fontFamily.loadType == 'fontFace') {
        fontFaceCSSStringContent = 'font-family: ' + fontFamily.key + '; src: url(' + fontFamily.url + ');';
      }
    });
  } else {
    // Leave it empty.
...
function setUtilityFontScale(env, task) {
  ...
  if (task == "build") {
    if (config.fontScale != null && config.fontScale != undefined && config.fontScale > 0) {
      return src(path + config.folders.styles + '/hydrogen/maps/_map-font-size.scss')
        .pipe(replace('$h2-font-scale: $H2FONTSCALE;', '$h2-font-scale: ' + config.fontScale + ';'))
