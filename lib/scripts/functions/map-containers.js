...
function createContainerSassVariables(env) {
  ...
  containerVariables = containerVariables.concat(
    '$h2-container-width-full: none;\n'
  );
...
function loadContainerMap(env) {
  ...
  containerConfig.forEach(function (containers) {
    var containersString =
      '"' + containers.key + '": "' + containers.maxWidth + '",';
    containerMapStringContent =
      containerMapStringContent.concat(containersString);
  });
  // Assemble the map.
  var containerMap =
    containerMapStringStart + containerMapStringContent + containerMapStringEnd;
  // Return the map.
  return containerMap;
}
...
function setContainerMap(env) {
  ...
  if (env == 'dev') {
    ...
    return src(
      './node_modules/@hydrogen-design-system/hydrogen.css/lib/styles/maps/_map-containers.scss'
    )
      .pipe(footer(containerMap))
