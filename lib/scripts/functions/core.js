...
function cacheH2Partials(env) {
  ...
  if (env == "dev") {
    return src([
      ...
      './lib/styles/utilities/_utility-width.scss'
    ])
    ...
    return src([
      ...
      './node_modules/@hydrogen-design-system/hydrogen.css/lib/styles/utilities/_utility-width.scss'
    ])
...
function preCleanCompress(env) {
  ...
  return src(path + config.folders.styles + '/hydrogen/compiled/utility.css')
    .pipe(postcss([cssnano({
      preset: ['lite']
    })]))
    .pipe(dest(path + config.folders.styles + '/hydrogen/compressed'));
