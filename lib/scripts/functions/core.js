...
function cleanH2Vars(env) {
  ...
  var outputDir = getH2OutputDirectory(env);
  var destPath = destRoot + outputDir;
  return del(destPath + '/hydrogen-variables.scss', {
    force: true,
  });
}
...
async function createH2Vars(env) {
  ...
  var outputDir = getH2OutputDirectory(env);
  var destPath = destRoot + outputDir;
  if (fs.existsSync(destPath + '/hydrogen-variables.scss') == true) {
    // Do nothing.
  } else {
    fs.writeFileSync(destPath + '/hydrogen-variables.scss', '');
  }
  await Promise.resolve('done?');
}
...
function cacheH2Core(env) {
  ...
  if (flexgridEnabled == true) {
    return src(sourceDir + 'lib/styles/core.scss')
      .pipe(
        replace(
    ...
    return src(sourceDir + 'lib/styles/core.scss').pipe(
...
function cacheStaticMaps(env) {
  ...
  var destPath = destRoot + outputDir;
  return src([
    sourceDir + 'lib/styles/maps/_map-font-size.scss',
    sourceDir + 'lib/styles/maps/_map-sides.scss',
    sourceDir + 'lib/styles/maps/_map-states.scss',
...
function cacheH2Build(env, task) {
  ...
  if (task == 'compile') {
    if (flexgridEnabled == true) {
      return src(sourceDir + 'lib/styles/compile.scss')
        .pipe(
          replace(
      ...
      return src(sourceDir + 'lib/styles/compile.scss').pipe(
        dest(destPath + '/hydrogen')
      );
    }
  } else if (task == 'build') {
    if (flexgridEnabled == true) {
      return src(sourceDir + 'lib/styles/build/**/*').pipe(
        dest(destPath + '/hydrogen/build')
      );
    } else {
      return src([
        sourceDir + 'lib/styles/build/**/*',
        '!' + sourceDir + 'lib/styles/build/flex-grid.scss',
        '!' + sourceDir + 'lib/styles/build/order.scss',
...
function cacheH2Partials(env) {
  ...
  if (flexgridEnabled == true) {
    return src(sourceDir + 'lib/styles/utilities/**/*')
      .pipe(
        replace(
          '$h2GridColumns: 12;',
    ...
    return src([
      sourceDir + 'lib/styles/utilities/**/*',
      '!' + sourceDir + 'lib/styles/utilities/_core-flex-grid.scss',
      '!' + sourceDir + 'lib/styles/utilities/_utility-flex-grid.scss',
      '!' + sourceDir + 'lib/styles/utilities/_utility-order.scss',
    ]).pipe(dest(destPath + '/hydrogen/utilities'));
  }
}
...
function compileUtility(env, task) {
  console.time(
    'Hydrogen '.gray + '[TIMELOG]'.magenta + ' Sass compile time was',
    'sassTime'
  );
  var destRoot = getH2DestinationPath(env);
  var outputDir = getH2OutputDirectory(env);
