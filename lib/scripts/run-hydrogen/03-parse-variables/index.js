...
function parse_variables(config) {
  try {
    ...
    if (config.themes['default']) {
      // Build the variables
      let root_default_variables = build_theme_variables(config, 'default', 'default');
      ...
      if (
        ...
        if (config.modes.dark.auto_apply_styles === true) {
          // Build the variables
          let root_dark_variables = build_theme_variables(config, 'default', 'dark');
          // Append it to the variable string
    ...
    if (config.themes) {
      Object.keys(config.themes).forEach(function (theme_key) {
        if (theme_key != 'default') {
          // Build the variables
          let theme_default_variables = build_theme_variables(config, theme_key, 'default');
          // Append it to the variable string
          ...
          if (
            ...
            if (config.modes.dark.auto_apply_styles === true) {
              // Build the variables
              let theme_dark_variables = build_theme_variables(config, theme_key, 'dark');
              // Append it to the variable string
        ...
        if (config.themes[theme_key].typography && config.themes[theme_key].typography.length > 0) {
          config.themes[theme_key].typography.forEach(function (type) {
            ...
            if (type.query != 'base') {
              ...
              let type_units = generate_type_units(config, type);
