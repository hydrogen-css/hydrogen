...
test('Valid type_config object using complete line_heights', () => {
  ...
  let type_config = {
    ...
    caption: {
      size: 'calc(var(--h2-font-size-body) / 1.125)',
      line_height: 1.4,
    },
    body: {
      size: '1rem',
  ...
  let output = {
    ...
    type_vars:
      ...
      '--h2-font-size-caption: calc(var(--h2-font-size-body) / 1.125);\n' +
      ...
      '--h2-font-size-h6: calc(var(--h2-font-size-body) * 1.125);\n' +
...
test('Returns properly when using the deprecated "copy" line-heights setting', () => {
  // Input / output
  /** @type {ParsedConfig} */
  let settings = get_parsed_config();
  /** @type {ParsedTypography} */
