...
test('That configure_themes returns wrappers as expected', () => {
  // Input
  /** @type {Config} */
  let defaults = get_default_config();
  /** @type {Config} */
  ...
  expect(
    output &&
      !output.testTheme.containers &&
      output.testTheme.wrappers &&
      output.testTheme.wrappers[0].key === 'testContainer'
  ).toBe(true);
});
