    ...
    if (config.media && config.media.queries && config.media.queries.length > 0) {
      ...
      config.media.queries.forEach((query) => {
        if (type_config.query_key === query.key) {
          // 2.0.6: Updated to reflect support for legacy formatting where the @rule was hardcoded; we need to check for the base query, however, as that key is unique and doesn't require the prepending logic
          if (query.query != 'base' && !query.query.match(/^@/g)) {
            type_config.query = '@media ' + query.query;
          } else {
