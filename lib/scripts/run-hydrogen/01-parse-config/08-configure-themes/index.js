  ...
  try {
    ...
    if (themes && themes.length > 0) {
      ...
      themes.forEach((theme, theme_index) => {
        ...
        // 2.0.6: Check to see if the theme contains deprecated container definitions and convert them to wrapper
        if (
          theme.containers &&
          theme.containers.length > 0 &&
          theme.wrappers &&
          theme.wrappers.length > 0
        ) {
          throw new Error(
            'It appears that you have configured both the deprecated "containers" setting and the newer "wrappers" setting. Please remove the deprecated setting.'
          );
        } else if (theme.containers && theme.containers.length > 0 && !theme.wrappers) {
          theme.wrappers = theme.containers;
          delete theme.containers;
        }
