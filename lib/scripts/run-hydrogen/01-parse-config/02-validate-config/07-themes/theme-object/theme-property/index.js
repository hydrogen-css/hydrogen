 ...
 * @returns {boolean}
 */
function validate_theme_property(defaults, config, theme, prop_model, parent) {
  try {
    let errors = [];
    ...
    if (theme[prop_model.property]) {
      ...
      // 2.0.6: Because container is deprecated in favor of wrapper, warn the user about using container
      if (prop_model.property === 'containers') {
        log_message({
          config: defaults,
          type: 'warning',
          step: 'Validating theme settings',
          message: 'The "containers" setting has been deprecated in favor of "wrappers".',
        });
      }
