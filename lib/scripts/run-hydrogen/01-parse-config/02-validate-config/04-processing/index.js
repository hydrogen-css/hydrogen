...
function validate_processing(settings) {
  try {
    ...
    if (
      settings.processing.error_recovery &&
      typeof settings.processing.error_recovery != 'boolean'
    ) {
      errors = errors.concat(
        new Error(
          'The "error_recovery" option in the "processing" section of your configuration must either be "true" or "false".'
        )
      );
    }
    if (
      settings.processing.write_css_file &&
      typeof settings.processing.write_css_file != 'boolean'
    ) {
      errors = errors.concat(
        new Error(
          'The "write_css_file" option in the "processing" section of your configuration must either be "true" or "false".'
        )
      );
    }
    if (errors.length === 0) {
