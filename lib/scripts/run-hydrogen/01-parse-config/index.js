...
function parse_config(args) {
  try {
    ...
    /** @type {Config} */
    let validated_config = validate_config(argv, defaults, config);
    ...
    if (argv.h2_env_dev === true || argv.h2_env_dev === 'true') {
      ...
      validated_config.processing.error_recovery = true;
    }
    // Prod
    ...
    if (
      argv.h2_logs === true ||
      argv.h2_logs === 'true' ||
      process.env.HYDROGEN_LOGS === true ||
      process.env.HYDROGEN_LOGS === 'true'
    ) {
      ...
      argv.h2_logs === false ||
      argv.h2_logs === 'false' ||
      process.env.HYDROGEN_LOGS === false ||
      process.env.HYDROGEN_LOGS === 'false'
    ) {
      validated_config.logging.generate_logs = false;
    }
    ...
    // Lightning CSS error recovery
    if (argv.h2_error_recovery === true || argv.h2_error_recovery === 'true') {
      validated_config.processing.error_recovery = true;
    } else if (argv.h2_error_recovery === false || argv.h2_error_recovery === 'false') {
      validated_config.processing.error_recovery = false;
    }
    // CSS file output
    if (argv.h2_write_css_file === true || argv.h2_write_css_file === 'true') {
      validated_config.processing.write_css_file = true;
    } else if (argv.h2_write_css_file === false || argv.h2_write_css_file === 'false') {
      validated_config.processing.write_css_file = false;
    }
