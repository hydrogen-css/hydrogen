...
function build_core(config, media_array, timer) {
  try {
    ...
    // const ordered = parsed_attribute_data.attributes;
    const ordered = Object.keys(parsed_attribute_data.attributes)
      .sort()
      .reduce((obj, key) => {
        obj[key] = parsed_attribute_data.attributes[key];
        return obj;
      }, {});
    // Loop through each property to process attribute instances
    Object.keys(ordered).forEach((property_key) => {
      // Loop through each attribute
      if (parsed_attribute_data) {
        ordered[property_key].forEach((attribute_instance) => {
    ...
    if (config.processing.write_css_file) {
      fs.writeFileSync(path.join(config.output.parsed.string + '/hydrogen.css'), processed_css);
    } else {
      log_message({
        type: 'info',
        step: 'Writing CSS',
        message: 'Hydrogen has skipped writing a CSS file based on your configuration.',
      });
    }
