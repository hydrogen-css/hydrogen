...
// Data imports
const { get_css_property_data } = require('../../../data/css-property-data');

// Local functions
const { wrapper_match } = require('./01-generate-namespace-regex');
const { attribute_match } = require('./02-generate-attribute-regex');
const { parse_queries } = require('./03-parse-attribute-queries');
 ...
 * @returns {{attribute_data: PropertyModel, namespace_wrapper: boolean}} The properties model populated with parsed data for all attributes used
 */
function parse_attributes(settings) {
  try {
    /** @type {PropertyModel} */
    let property_data = get_css_property_data();
    let namespace_wrapper = false;
    ...
    if (!fs.existsSync(settings.output.parsed.string)) {
      throw new Error(
        "The following output directory doesn't exist: " + settings.output.parsed.string
      );
    }
    ...
    let ignored_files = ['hydrogen.css', 'hydrogen.raw.css', 'hydrogen.vars.css'];
    ...
    assembled_ignored_paths.forEach((path) => {
      sanitized_ignored_paths = sanitized_ignored_paths.concat(path.replace(/\\/g, '/'));
    });
    // Assemble the user's folder paths and the complete ignored file paths into a usable glob
    let user_input_paths_with_ignored_files = glob.sync('{' + sanitized_input_glob + '}', {
    ...
    if (user_input_paths_with_ignored_files) {
      ...
      let hydrogen_wrapper_attribute_regex = wrapper_match();
      // Create a RegEx for matching Hydrogen style attributes
      let hydrogen_style_attribute_regex = attribute_match();
      ...
      for (let file in user_input_paths_with_ignored_files) {
        ...
        let file_path_string = user_input_paths_with_ignored_files[file].toString();
        ...
        if (fs.lstatSync(attribute_file).isDirectory() === false) {
          ...
          if (
            !attribute_file.match(/\.css$/gm) &&
            file_data.match(hydrogen_wrapper_attribute_regex) != null
          ) {
            namespace_wrapper = true;
          }
          ...
          let attributes = [];
          while ((match_array = hydrogen_style_attribute_regex.exec(file_data)) !== null) {
      ...
      if (settings.logging.verbose_console_output === true) {
      ...
      return {
        attribute_data: property_data,
        namespace_wrapper: namespace_wrapper,
      };
    } else {
