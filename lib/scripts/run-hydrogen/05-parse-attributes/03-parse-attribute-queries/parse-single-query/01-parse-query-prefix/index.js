              ...
              if (theme_check === false) {
                ...
                if (modifier.startsWith('selectors[') === true) {
                  ...
                  if (property_string === 'flex-grid') {
                    ...
                    // Parse any interaction states that are being applied to the nested elements
                    state_data.forEach((state) => {
                      let state_query = new RegExp(state + '(?!-)', 'g');
                      if (children.match(state_query)) {
                        parsed_child_states = parsed_child_states.concat(state);
                      }
                    });
                  }
