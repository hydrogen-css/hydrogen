 ...
 * @param {string} character The character to separate the list by
 * @returns {string[]}
 */
function parse_options(settings, attribute, query, file, input, character) {
  try {
    ...
    for (let i = 0; i < input.length; ) {
      ...
      if (input.charAt(i) === character && bracket_count === 0 && square_count === 0) {
    ...
    if (parsed_input.length === 0) {
      throw new Error('This query has no options passed to it, so it has been ignored.');
    }
    ...
    if (square_count != 0 || bracket_count != 0) {
      throw new Error('This query has an unequal number of brackets. This is likely a typo.');
    } else {
      return parsed_input;
    }
  } catch (error) {
