...
// Local functions
const { parse_query } = require('./parse-single-query');
 ...
 * @returns {ParsedQuery[]}
 */
function parse_queries(settings, property_data, property, attribute, input, file) {
  try {
    ...
    for (let i = 0; i < input.length; ) {
      ...
      if (input.charAt(i) === ')' && bracket_count === 0 && square_count === 0) {
    ...
    if (query_array.length === 0) {
      throw new Error('This attribute has no queries passed to it, so it has been ignored.');
    }
    // Loop through the queries that were found and parse them, after which return them
    query_array.forEach(function (query) {
      try {
        let parsed_query = parse_query(settings, property_data, property, attribute, query, file);
