...
function checkDuplicates(property, property_array) {
...
function validateConfig() {
  try {
    ...
    if (
      fs.existsSync(path.resolve(process.cwd(), 'hydrogen.config.json')) != true
    ) {
      if (
        fs.existsSync(
      ...
      configuration_path = path.resolve(process.cwd(), 'hydrogen.config.json');
    }
    ...
    h2_error_detail('configuration', null, [configuration_path], error);
    return false;
  }
}
