...
function buildTypography(settings) {
  try {
    var typeSettings = settings.styles.foundations.typography;
    var typeArray = [];
    typeSettings.forEach(function (typeSetting) {
      ...
      if (mediaMatch == true) {
        ...
        log_info(
          ...
          null,
          [settings.path],
          'A typography query_key (' +
            typeSetting.query_key +
            ') does not match any of the media query keys in your configuration.'
        );
        return false;
      }
      ...
      settingObject.display = {};
      var displaySize = h1Size * type_scale;
      settingObject.display.size =
        'calc(var(--h2-font-size-h1) * ' + type_scale + ' )';
      ...
      var display_line_height = calculate_line_height(displaySize, line_height);
      settingObject.display.line_height = display_line_height;
