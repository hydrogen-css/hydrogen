  ...
  try {
    ...
    try {
      ...
      if (settings.debug != null && settings.debug === false) {
        ...
        if (
          fs.existsSync(path.join(userOutput + '/hydrogen.logs.media.json')) ==
          true
        ) {
        ...
        if (
          fs.existsSync(
            path.join(userOutput + '/hydrogen.logs.attributes.json')
          ) == true
        ) {
          fs.unlinkSync(
            path.join(userOutput + '/hydrogen.logs.attributes.json')
        ...
        if (
          fs.existsSync(
            path.join(userOutput + '/hydrogen.logs.values.json') == true
          )
        ) {
      ...
      h2_error(
        "Hydrogen's output contained invalid CSS that PostCSS could not process. This is likely a typo in one of your attributes. Please check the hydrogen.raw.css file in your output folder for syntax errors to pinpoint the exact problem(s)."
      );
