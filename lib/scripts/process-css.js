...
function process_hydrogen(settings) {
  try {
    ...
    if (fs.existsSync(path.join(settings.output.parsed.string + '/hydrogen.css')) == true) {
      fs.unlinkSync(path.join(settings.output.parsed.string + '/hydrogen.css'));
    }
    // Load the raw CSS synchronously
    let rawCSS = fs.readFileSync(path.join(settings.output.parsed.string + '/hydrogen.raw.css'));
      ...
      result.messages.forEach(function (m, index) {
        if (m.type === 'warning' || m.type === 'error') {
          ...
          log_message({
            ...
            files: [path.resolve(settings.output.parsed.string + '/hydrogen.css')],
          });
        }
      });
