...
function process_hydrogen(settings, command, timer_start_total_build) {
  try {
    ...
    if (
      fs.existsSync(
        path.join(settings.runtime.output.string + '/hydrogen.css')
      ) == true
    ) {
      fs.unlinkSync(
        path.join(settings.runtime.output.string + '/hydrogen.css')
    ...
    try {
      ...
      postcss(plugin_array)
        .process(rawCSS, {
          from: path.join(settings.runtime.output.string + '/hydrogen.raw.css'),
          to: path.join(settings.runtime.output.string + '/hydrogen.css'),
        })
        .then((result) => {
          result.messages.forEach(function (m, index) {
            if (m.type === 'warning' || m.type === 'error') {
              ...
              log_message({
                ...
                files: [
                  path.resolve(
                    settings.runtime.output.string + '/hydrogen.css'
                  ),
                ],
              });
            }
          ...
          if (result.map) {
            fs.writeFileSync(
              path.join(settings.runtime.output.string + '/hydrogen.css.map'),
              result.map.toString()
            );
          }
          ...
          if (settings.build.var_export === true) {
            log_message({
              ...
              files: [
                settings.runtime.output.string + '/hydrogen.css',
                settings.runtime.output.string + '/hydrogen.vars.css',
              ],
            });
          } else {
            log_message({
              ...
              files: [settings.runtime.output.string + '/hydrogen.css'],
            });
          }
          if (command === 'watch') {
            log_message({
      ...
      log_message({
        ...
        files: [
          path.resolve(settings.runtime.output.string + '/hydrogen.raw.css'),
        ],
      });
      return false;
    }
