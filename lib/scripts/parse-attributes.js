 ...
 */
function parse_list(attribute, query, file, input, character) {
  let parsed_input = [];
      ...
      parsed_modifier_array.forEach(function (modifier) {
        ...
        if (media_check === false) {
          ...
          if (mode_check === false) {
            ...
            // The modifier wasn't a state value, so check for selectors or children
...
function parse_options(settings, property, attribute, query, input, file) {
  try {
    ...
    let options_array = parse_list(attribute, query, file, input, ',');
    if (options_array) {
