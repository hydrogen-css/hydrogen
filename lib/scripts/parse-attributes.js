 ...
 * @returns {{children: string[], class: string, id: string, media: string, mode: string, state: string} | false} An object containing parsed modifiers
 */
function parse_modifiers(settings, property, attribute, input, file) {
  try {
      ...
      parsed_modifier_array.forEach(function (modifier) {
        ...
        if (modifier.startsWith('id[') === true) {
          ...
          if (ids[1].trim() === '') {
            log_message({
              type: 'warning',
              message:
                'This attribute contains a query using an empty :id[] modifier, so this modifier has been ignored.',
        ...
        if (modifier.startsWith('class[') === true) {
          ...
          if (classes[1].trim() === '') {
            log_message({
              type: 'warning',
              message:
                'This attribute contains a query using an empty :class[] modifier, so this modifier has been ignored.',
        ...
        if (modifier.startsWith('children[') === true) {
          ...
          if (property === 'flex-grid') {
            ...
            let children = modifier.match(/\[([^\]]*)/);
            // Check to see if the children group was empty, and if it was, throw an error, otherwise, pass the parsed children
            if (children[1].trim() === '') {
              log_message({
                type: 'warning',
                message:
                  'This attribute contains a query using an empty :children[] modifier, so this modifier has been ignored.',
                step: 'Parsing query children modifiers',
                attribute: attribute,
                files: [file],
              });
            } else {
              // Split the children array by commas
              let child_array = children[1].split(',');
              // Assemble the children selector
              child_array.forEach(function (item) {
                parsed_children = parsed_children.concat(item.trim());
              });
            }
          }
        }
 ...
 */
function parse_options(settings, property, attribute, input, file) {
  try {
...
function parse_modifiers_and_options(
  ...
  try {
    ...
    log_message({
      type: 'error',
      message: error,
      step: 'Parsing query for modifiers and options',
      attribute: attribute,
      files: [file],
    });
    return false;
...
function parse_instance_queries(settings, property, attribute, input, file) {
  try {
    ...
    query_array.forEach(function (query) {
      ...
      // If a parsed query was found, add it to the final query array
      if (parsed_query) {
        queries.push({
...
function parse_input_data_for_attributes(settings) {
  try {
    ...
    let user_input_paths_as_glob = settings.runtime.input.glob;
    let user_input_paths_as_array = settings.runtime.input.array;
    ...
    for (let file in user_input_paths_with_ignored_files) {
      ...
      if (fs.lstatSync(attribute_file).isDirectory() == false) {
        ...
        while (
          ...
          if (match_array.index === hydrogen_style_attribute_regex.lastIndex) {
        ...
        if (attributes.length > 0) {
          ...
          attributes.forEach(function (match) {
                ...
                if (prop_key.name === property) {
                  ...
                  prop_key.instances.forEach(function (prop_instance) {
                    if (prop_instance.attribute === attribute) {
                      ...
                      if (
                        prop_instance.files.includes(file_path_string) === false
                      ) {
                        prop_instance.files =
            ...
            // Log an error for any invalid properties
            if (validity === false) {
              log_message({
                type: 'error',
                message:
                  "This attribute contains a property that isn't supported by Hydrogen. Check the documentation website for a list of supported properties.",
    ...
    if (settings.build.logs === true) {
      fs.writeFileSync(
        path.join(settings.runtime.logging.directory + '/property-data.json'),
