 ...
 */
function parse_color_variable(settings, color, mode) {
  try {
    let color_variables = '';
    ...
    log_message({
      type: 'error',
      settings: settings,
      step: 'Color variable parsing',
      message:
        "A color defined in your configuration is invalid. If you're using CSS keyword colors, double check to make sure the keyword used is a valid option.",
      error: error,
    });
    return false;
  }
 ...
 */
function build_size_and_line_height(settings, type_setting) {
  try {
    // Captions
    // prettier-ignore
      ...
      if (
        ...
        settings.themes[theme].typography.forEach(function (type) {
          ...
          if (type.query === 'base') {
            ...
            let new_type = build_size_and_line_height(settings, type);
            if (new_type) {
    ...
    if (
      ...
      settings.themes[theme].colors.forEach(function (color) {
        if (color[mode] && color[mode].color) {
          let new_color = parse_color_variable(settings, color, mode);
          if (new_color) {
            // prettier-ignore
    ...
    if (
      ...
      settings.themes[theme].gradients.forEach(function (gradient) {
        if (
          ...
          if (mode === 'default') {
            // prettier-ignore
            gradient_variables = gradient_variables + '--h2-gradient-' + gradient.key + '-locked: ' + gradient[mode].gradient + ';\n';
            // prettier-ignore
            gradient_variables = gradient_variables + '--h2-gradient-fallback-' + gradient.key + '-locked: ' + gradient[mode].fallback + ';\n';
          }
