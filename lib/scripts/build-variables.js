 ...
 */
function build_theme_variables(
  settings,
  theme,
  mode,
  core_units,
  ...
  try {
    ...
    if (theme === 'default' && mode === 'default') {
      ...
      if (settings.build.modes.dark.method === 'preference') {
        // prettier-ignore
        root_start = '\n@media screen and (prefers-color-scheme: dark) {\n:root{\n';
        root_close = '}\n}';
      } else if (settings.build.modes.dark.method === 'toggle') {
        root_start = '\n[data-h2*="dark"]{\n';
        root_close = '}';
      }
    ...
    // Colors
    if (
      settings.runtime.themes[theme].colors &&
      settings.runtime.themes[theme].colors.length > 0
    ) {
      colors = colors + '/* Colors */\n';
      settings.runtime.themes[theme].colors.forEach(function (color) {
    ...
    // Containers
    if (
      settings.runtime.themes[theme].containers &&
      settings.runtime.themes[theme].containers.length > 0
    ) {
      containers = containers + '/* Container widths */\n';
      settings.runtime.themes[theme].containers.forEach(function (container) {
    ...
    // Font families
    if (
      settings.runtime.themes[theme].fonts &&
      settings.runtime.themes[theme].fonts.length > 0
    ) {
      fonts = fonts + '/* Font families */\n';
      settings.runtime.themes[theme].fonts.forEach(function (font) {
    ...
    // Gradients
    if (
      settings.runtime.themes[theme].gradients &&
      settings.runtime.themes[theme].gradients.length > 0
    ) {
      gradients = gradients + '/* Gradients */\n';
      settings.runtime.themes[theme].gradients.forEach(function (gradient) {
        if (
          gradient[mode] &&
    ...
    // Radii
    if (
      settings.runtime.themes[theme].radii &&
      settings.runtime.themes[theme].radii.length > 0
    ) {
      radii = radii + '/* Radii */\n';
      settings.runtime.themes[theme].radii.forEach(function (radius) {
    ...
    if (
      ...
      settings.runtime.themes[theme].shadows.forEach(function (shadow) {
...
function build_variables(settings) {
  try {
    ...
    let core_units = '/* Core units */\n';
    settings.styles.foundations.typography.forEach(function (type) {
      if (type.query === settings.build.base_query_key) {
        // prettier-ignore
        core_units = core_units + '--h2-base-font-size: ' + type.size + ';\n';
        // prettier-ignore
        core_units = core_units + '--h2-base-line-height: ' + type.line_height + ';\n';
        // prettier-ignore
        core_units = core_units + '--h2-base-whitespace: ' + type.line_height + ';\n';
      }
    });
    // Build the root typography
    let typography = '/* Font sizes and line heights */\n';
    settings.styles.foundations.typography.forEach(function (type) {
      if (type.query === settings.build.base_query_key) {
    ...
    if (settings.runtime.themes['default']) {
      // Build the variables
      ...
      if (
        ...
        settings.build.modes.dark.automatic &&
        settings.build.modes.dark.method
      ) {
        if (settings.build.modes.dark.automatic === true) {
          // Build the variables
    ...
    if (settings.runtime.themes) {
      Object.keys(settings.runtime.themes).forEach(function (theme_key) {
