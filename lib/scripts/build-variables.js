...
function buildVariables(argv, format) {
  try {
    ...
    if (fs.existsSync(getUserOutput(argv, 'string') + '/hydrogen.vars.' + format) == true) {
      fs.unlinkSync(getUserOutput(argv, 'string') + '/hydrogen.vars.' + format);
    }
    // Write the file
    fs.writeFileSync(getUserOutput(argv, 'string') + '/hydrogen.vars.' + format, variableString);
    // End the timer
