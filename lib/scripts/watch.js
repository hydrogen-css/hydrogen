...
function loadHydrogenConfig(done) {
  ...
  if (fs.existsSync('./hydrogen.config.json') == false) {
    console.error('Hydrogen: you do not have a hydrogen.config.json file in your project root. Please create one to continue.');
    return false;
  } else {
    // Import the user's JSON config file.
    config = JSON.parse(fs.readFileSync('./hydrogen.config.json'));
  }
  if (config.styles == null || config.styles == undefined || config.styles == "" || config.styles.path == null || config.styles.path == undefined || config.styles.path == "") {
    console.error('Hydrogen: please ensure you define a style path in your configuration file before continuing.');
    return false;
  }
  if (config.markup == null || config.markup == undefined || config.markup == "" || config.markup.path == null || config.markup.path == undefined || config.markup.path == "") {
    console.error('Hydrogen: please ensure you define a markup path in your configuration file before continuing.');
    return false;
  }
  done();
}
...
function cleanHydrogen() {
  return del('./' + config.styles.path + '/hydrogen.css');
}
...
function createDevelopmentCSS(done) {
  ...
  console.log('Hydrogen: you\'ve successfully compiled Hydrogen. We\'re now watching for changes to your config.');
  done();
}
