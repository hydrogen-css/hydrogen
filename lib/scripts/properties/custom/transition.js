  ...
  try {
    ...
    if (options.length >= 2) {
      ...
      if (
        ...
        Object.keys(settings.themes[theme].transitions).length > 0 &&
        settings.themes[theme].transitions.durations &&
        settings.themes[theme].transitions.durations.length > 0
      ) {
        settings.themes[theme].transitions.durations.forEach(function (
          duration_setting
        ) {
          if (option2 === duration_setting.key) {
            if (prop_value.modifiers.mode === 'all') {
              // prettier-ignore
              option2 = 'var(' + '--h2-transition-duration-' + duration_setting.key + '-locked)';
            } else {
              // prettier-ignore
              option2 = 'var(' + '--h2-transition-duration-' + duration_setting.key + ')';
            }
          }
        });
      }
      ...
      if (
        ...
        settings.themes[theme].transitions.functions.forEach(function (
          ...
          if (option3 == function_setting.key) {
            if (prop_value.modifiers.mode === 'all') {
              // prettier-ignore
              option3 = 'var(' + '--h2-transition-function-' + function_setting.key + '-locked)';
            } else {
              // prettier-ignore
              option3 = 'var(' + '--h2-transition-function-' + function_setting.key + ')';
            }
          }
        });
      }
    ...
    if (options.length >= 4) {
      ...
      if (
        ...
        settings.themes[theme].transitions.delays.forEach(function (
          delay_setting
        ) {
          if (option4 == delay_setting.key) {
            if (prop_value.modifiers.mode === 'all') {
              // prettier-ignore
              option4 = 'var(' + '--h2-transition-delay-' + delay_setting.key + '-locked)';
            } else {
              // prettier-ignore
              option4 = 'var(' + '--h2-transition-delay-' + delay_setting.key + ')';
            }
          }
        });
      }
