...
function parse_flex_grid(instance, property, selector, values) {
  try {
    ...
    // -------------------------------------------------------------------------
    // Value 2: [required] gutter or column gutter
    var gutter;
    var gutter_error_message = '';
    if (values.length >= 2) {
      gutter = parse_whitespace_value(instance, property, values[1]);
    }
    // -------------------------------------------------------------------------
    // Value 3: [optional] row gutter
    var row_gutter;
    var row_gutter_error_message = '';
    if (values.length >= 3) {
      row_gutter = parse_whitespace_value(instance, property, values[2]);
    }
    // =========================================================================
    ...
    if (values.length === 2) {
      ...
      h2_error_detail(
        ...
        instance.attribute,
        instance.files,
        "flex-grid accepts 2 (alignment, gutter) or 3 (alignment, column_gutter, row_gutter) options and you've specified " +
          values.length +
          '.'
      );
      return null;
