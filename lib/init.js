...
// Set Prompt settings
prompt.message = 'Hydrogen ' + colors.blue('[SETUP]');
...
function setFolders(argv) {
  try {
    ...
    if (fs.existsSync(envSrc + 'hydrogen.config.json') == false) {
      console.log('Hydrogen', '[SETUP]'.blue, 'Welcome to Hydrogen! ☀️');
      console.log(
        'Hydrogen',
        '[SETUP]'.blue,
    ...
    console.error('Hydrogen', '[ERROR]'.red, err);
    return false;
  }
}
...
function createHydrogenInit(argv) {
  try {
    ...
    var defaults = JSON.parse(
      fs.readFileSync(
        path.resolve(__dirname, 'data/hydrogen.defaults.json')
      )
    );;
